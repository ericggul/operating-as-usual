  ...
  return (
    <div style={{ width: "100vw", height: "100vh" }}>
      <Canvas camera={{ position: [10, 10, 10] }}>
        ...
        {new Array(60).fill(0).map((_, j) => (
          <group key={j} rotation={[getRandom(0, Math.PI), getRandom(0, Math.PI), getRandom(0, Math.PI)]}>
            {new Array(30).fill(0).map((_, i) => (
              <mesh key={i} position={[0, Math.sqrt(2) * i, 0]} rotation={[Math.PI / 4, 0, Math.PI / 4]}>
                <boxGeometry attach="geometry" args={[Math.sqrt(i), Math.sqrt(i), Math.sqrt(i)]} />
                <meshStandardMaterial attach="material" color={`hsl(${j * 10}, 100%, 80%)`} />
              </mesh>
            ))}
          </group>
        ))}
