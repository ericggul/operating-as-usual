        ...
        {new Array(100).fill(0).map((_, j) => (
          <group key={j} rotation={[getRandom(0, Math.PI), getRandom(0, Math.PI), getRandom(0, Math.PI)]}>
            {new Array(30).fill(0).map((_, i) => (
              <mesh key={i} position={[0, Math.sqrt(2) * (i + 6), 0]}>
                <boxGeometry attach="geometry" args={[Math.sqrt(i), Math.sqrt(i), Math.sqrt(i)]} rotation={[getRandom(0, Math.PI), getRandom(0, Math.PI), getRandom(0, Math.PI)]} />
                <meshStandardMaterial attach="material" color={`hsl(${getRandom(200, 300)}, 100%, 80%)`} />
              </mesh>
            ))}
          </group>
        ))}
